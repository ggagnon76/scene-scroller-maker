...
export class Texture2Polygon {
    ...
    async generateDrawingPolygon() {
        let confirm = await this.#generateMaskFromOtherSubScenes();
        if ( !confirm ) return false;
        confirm = await this.#getPixelData();
        if ( !confirm ) return false;
        ...
        if ( !confirm ) return false;
        confirm = await this.#removeNonPerimeterWalls();
        if ( !confirm ) return false;
        ...
        if ( !confirm ) return false;
        confirm = this.#linkFinalVertices();
        if ( !confirm ) return false;
    ...
    async #generateMaskFromOtherSubScenes() {
        ...
        if ( this.#isDebugging ) {
            ...
            const title = "Review Mask Texture.";
            const content = "<p>Click YES to continue if explored texture to omit (black) is correct.</p><p>Click NO to abort execution if texture is incorrect.</p>";
    ...
    async #getPixelData() {
        ...
        if ( this.#isDebugging ) {
            ...
            const content = "<p>Click YES to continue if explored texture is correct.</p><p>Click NO to abort execution if texture is incorrect.</p>";
            const confirm = await this.confirmDialog(title, content);
    ...
    #compressPixelData() {
        ...
        // would be the last in this.pixel since we are iterating backwards).
        // When we find it, record the pixel coordinate and the array index.  Then break out of the loop.
        ...
        // Substract 1 from coord.x because the first thing the loop will do is add 1.
        coord.x -= 1;
        ...
        for (let i=firstWhite.index; i <= lastWhite.index; i+=4) {
            ...
            // Checks when the pixel is white
            if ( pixelIsWhite ) {
            ...
            // Chekcs when the pixel is black
            if ( pixelIsBlack ) {
    ...
    async #removeNonPerimeterWalls() {
        ...
        if ( this.#isDebugging ) {
            ...
            const title = "Review Vertices after inner and dangling walls removed.";
            const content = "<p>Click YES to continue if vertices are correct.</p><p>Otherwise, click NO to abort execution.</p>";
            const confirm = await this.confirmDialog(title, content);
     ...
     */
     async #prepareVertexData() {
        ...
        this.boundingBox = {
            x: smallestX,
            y: smallestY,
            sceneX: smallestX - this.dims.paddingX,
            sceneY: smallestY - this.dims.paddingY,
