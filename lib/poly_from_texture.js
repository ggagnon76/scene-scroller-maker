...
export class Texture2Polygon {
    ...
    getColor(box, coord) {
        ...
        const d = canvas.dimensions;
        const R = Math.round((coord.x / d.sceneWidth) * 255);
        const G = Math.round((coord.y / d.sceneHeight) * 255);
        ...
        const slope = (coord.x - boxCenterPoint.x) / (coord.y - boxCenterPoint.y);
        let b = coord.y - (slope * coord.x);
        let boundaryCoord = {x: undefined, y: undefined};
        if ( slope < 0 && coord.x < boxCenterPoint.x ) {
        ...
        const B = Math.round((  lineLength({x: coord.x, y: coord.y}, {x: this.boundingBox.x, y: this.boundingBox.y}) /
                                lineLength({x: boundaryCoord.x, y: boundaryCoord.y}, {x: this.boundingBox.x, y: this.boundingBox.y})) *
                                255);
                                
        // Have to convert RGBA to HEX...  Foundry has util for this?
    }
}