...
export class Texture2Polygon {
    ...
    #compressPixelDataTemp() {
        ...
        for (let i=0; i < this.pixels.length; i+=4) {
            ...
            coord.x += 1
            if ( coord.x === this.dims.sceneWidth ) {
                coord.y += 1;
                coord.x = 0;
            }
            ...
            if ( pixelIsWhite ) {
                ...
                if ( this.compressedPixelData.vertical.has(x) ) {
                    ...
                    const column = this.compressedPixelData.vertical.get(x);
                    const lastArray = column[column.length - 1];
                    if ( lastArray.length === 2 ) {
                        column.push([y]);
                        this.compressedPixelData.vertical.set(x, column);
                    }
                ...
                else this.compressedPixelData.vertical.set(x, [y]);
                ...
                if ( this.compressedPixelData.horizontal.has(y) ) {
                    ...
                    const row = this.compressedPixelData.horizontal.get(y);
                    const lastArray = row[row.length - 1];
                    if ( lastArray.length === 2 ) {
                        row.push([x]);
                        this.compressedPixelData.horizontal.set(y, row);
                    }
                ...
                else this.compressedPixelData.horizontal.set(y, [x]);
            }
            ...
            // Checks if y = sceneHeight - 1
            if ( y === this.dims.sceneHeight - 1 ) {
