...
export class Texture2Polygon {
    ...
    #compressPixelData() {
        ...
        function _analyzePixels(isBlack, x, y, direction) {
            const pixelIndex = ((y * d.sceneWidth + x) * 4);
        ...
        for (let x = 0; x < d.sceneWidth; x++) {
            for (let y = 0; y < d.sceneHeight; y++) {
                isBlack = analyzePixels(isBlack, x, y, "vertical");
            }

            if ( !isBlack ) {
                const toUpdate = this.compressedPixelData.vertical.get(x);
                toUpdate[toUpdate.length-1].push(d.sceneHeight-1);
        ...
        for (let y = 0; y < d.sceneHeight; y++) {
            ...
            if ( !isBlack ) {
                ...
                toUpdate[toUpdate.length-1].push(d.sceneWidth-1);
        ...
        for (const [id, wall] of this.instanceWalls) {
            for (let i=0; i < wall.data.c.length; i+= 2) {
                ...
                if ( !checkisExplored ) {
                    ...
                    this.vertexCoords.delete(JSON.stringify([column + d.paddingX, row + d.paddingY]));
                    break;
                }
            }
        }
    ...
    async #removeInteriorWallsVertices() {
        ...
        for (const vertex of this.vertexCoords) {
            ...
            const finalVertex = [vert[0] - d.paddingX, vert[1] - d.paddingY];
            const check = this.perimeterPixels.has(JSON.stringify(finalVertex));
            if ( !check ) {
                /**
                ...
                for (const offsetX of pixelOffset) {
                    ...
                    for (const offsetY of pixelOffset) {
                        ...
                        const offsetPixel = [finalVertex[0] + offsetX, finalVertex[1] + offsetY];
                        foundPerimeterPixel = this.perimeterPixels.has(JSON.stringify(offsetPixel));
                        if ( foundPerimeterPixel ) this.perimeterPixels.add(JSON.stringify(finalVertex));
    ...
    async #addBoundaryVertices() {
        const d = canvas.dimensions;
        const boundaries = [["horizontal", [0, d.sceneHeight - 1]], ["vertical", [0, d.sceneWidth - 1]]];
            ...
            for (const bounds of boundary[1]) {
                if ( this.compressedPixelData[boundary[0]].has(bounds) ) {
                    ...
                    for (const segment of boundarySegments) {
                        const coord1 = boundary[0] === "horizontal" ? [segment[0], bounds] : [bounds, segment[0]];
                        const coord2 = boundary[0] === "horizontal" ? [segment[1], bounds] : [bounds, segment[1]];
                        const coords = [coord1, coord2];
                        for (const coord of coords) {
                            const otherCoord = JSON.stringify(coord) === JSON.stringify(coord1) ? coord2 : coord1
                            const vertexFromOtherCoord = [otherCoord[0] + d.paddingX, otherCoord[1] + d.paddingY];
                            ...
                            if ( vertex.length ) {
                                ...
                                if ( vertex[0].second !== undefined) {
                                    ...
                                    vertex[0].second = vertexFromOtherCoord;
                                    continue;
                                }
                            }
                            // This vertex doesn't exist.
                            const vertexFromCoord = [coord[0] + d.paddingX, coord[1] + d.paddingY];
                            this.vertexCoords.push({
                                coord: JSON.stringify(vertexFromCoord),
                                walls: [],
                                first: vertexFromOtherCoord,
                                second: undefined
                            })
                        }
                    }
