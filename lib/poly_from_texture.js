...
export class Texture2Polygon {
    constructor(parent) {
        ...
        this.instanceWalls = this.#getWallsFromVertices();
    ...
    #getWallsFromVertices() {
        const vertices = [...this.vertices].map(v => JSON.parse(v));
        const walls = canvas.walls.placeables;
        const result = new Set();
        for (const vertex of vertices) {
            for (const wall of walls) {
                if (    (wall.document.c[0] !== vertex.x || wall.document.c[1] !== vertex.y) &&
                        (wall.document.c[2] !== vertex.x || wall.document.c[3] !== vertex.y) ) continue;
                result.add(wall);
            }
        }
        return result
    ...
    async #generateMaskFromOtherSubScenes() {
        ...
        if ( this.#isDebugging ) {
            ...
            tempSprite.position = {x: this.dims.sceneX, y: this.dims.sceneY};
    ...
    async #getPixelData() {
        ...
        this.pixels = canvas.app.renderer.extract.pixels(canvas.fog.sprite.texture);
    ...
    #compressPixelData() {
        ...
        for (let i=0; i < this.pixels.length; i+=4) {
            coord.x += 1;
            if ( coord.x === this.dims.sceneX + this.dims.sceneWidth ) {
                coord.y += 1;
                coord.x = this.dims.sceneX;
            }
        ...
        coord = {
            x: this.dims.sceneX + this.dims.sceneWidth + 1,
            y: this.dims.sceneY + this.dims.sceneHeight
        }
        ...
        for (let i=this.pixels.length - 1; i >= 0; i-=4) {
            coord.x -= 1;
            if ( coord.x === this.dims.sceneX ) {
                coord.y -= 1;
                coord.x = this.dims.sceneX + this.dims.sceneWidth
            }
        ...
        for (let i=firstWhite.index; i <= lastWhite.index; i+=4) {
            ...
            if ( coord.x === this.dims.sceneX + this.dims.sceneWidth - 1 ) {
    ...
    async #extractVerticesFromWalls() {
        for (const wall of this.instanceWalls) {
            this.vertexCoords.add(JSON.stringify([wall.document.c[0], wall.document.c[1]]));
            this.vertexCoords.add(JSON.stringify([wall.document.c[2], wall.document.c[3]]));
        }
    ...
    async #removeNonPerimeterWalls() {
        for (const wall of this.instanceWalls) {
            for (let i=0; i < wall.document.c.length; i+= 2) {
                const row = wall.document.c[i+1];
                const column = wall.document.c[i]; 
                ...
                if ( !check ) {
                    ...
                    if ( !foundPerimeterPixel ) {
                        ...
                        if ( checkisExplored ) {
                            ...
                            this.instanceWalls.delete(wall);
                            this.vertexCoords.delete(JSON.stringify([column, row]));
                        }
                    }
                }
    ...
    #linkVerticesByWalls() {
        ...
        for (const vertexString of this.vertexCoords) {
            ...
            for (const wall of this.instanceWalls) {
                const c1 = JSON.stringify([wall.document.c[0], wall.document.c[1]]);
                const c2 = JSON.stringify([wall.document.c[2], wall.document.c[3]]);
        ...
        for (const vertexObj of newVertexArray) {
            ...
            for (let i=0; i < vertexObj.walls.length; i++) {
                ...
                const c1 = [wall.document.c[0], wall.document.c[1]];
                const c2 = [wall.document.c[2], wall.document.c[3]];
    ...
    async #addBoundaryVertices() {
        ...
        function _addBoundaryVertex(dir, limits) {
            ...
            const padPlusWidth = this.dims.sceneWidth + this.dims.sceneX - 1;
            const padPlusHeight = this.dims.sceneHeight + this.dims.sceneY - 1;
        ...
        addBoundaryVertex("horizontal", [this.dims.sceneY, this.dims.sceneHeight + this.dims.sceneY - 1]);
        addBoundaryVertex("vertical", [this.dims.sceneX, this.dims.sceneWidth + this.dims.sceneX - 1])
    ...
    #generateBoundingBox() {
        const smallestX = this.vertexCoords.reduce((pv, cv) => Math.min(pv, JSON.parse(cv.coord)[0]), this.dims.sceneWidth + this.dims.sceneX + 1);
        const smallestY = this.vertexCoords.reduce((pv, cv) => Math.min(pv, JSON.parse(cv.coord)[1]), this.dims.sceneHeight + this.dims.sceneY + 1);
    ...
    getColor(box) {
        ...
        const sceneCenter = {
            x: this.dims.sceneX + this.dims.sceneWidth / 2,
            y: this.dims.sceneY + this.dims.sceneHeight / 2
        }
        ...
        const maxB = lineLength(sceneCenter, {x: this.dims.sceneX, y: this.dims.sceneY});
    ...
    async #generateDrawingPolygon() {
        ...
        document.shape.points = flattenPoints(this.polygonVertexCoordinates);
