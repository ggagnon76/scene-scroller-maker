...
export class Texture2Polygon {
    ...
    static getVertices() {
        ...
        function _getAdjacentPoint(vertex) {
        ...
        function fillArray(arr, coord = null) {
            if ( arr.length === 0 && coord === null) log(false, "fillArray function missing coordinates for first pass.");
            if ( arr.length === edgeThreshold ) return arr;
            
            const newCoord = arr.length === 0 ? getAdjacentPoint(coord) : getAdjacentPoint(arr[arr.length - 1]);
        ...
        this.vertices.forEach((value, vertexCoord) => {
            ...
            // edgeThreshold has been defined already (above)
            // Going to populate two arrays of sequential point coordinates
            // The last point in the first array is also the first point in the second array.
            ptsBetweenVertices = [];
            let testSeries = [];
            let newPoint = []

            fillArray(ptsBetweenVertices, vertexCoord)
