...
export class Texture2Polygon {
    ...
    static getVertices() {
        ...
        for (const [key, value] of this.vertices) {
            ...
            for (const dir of directions) {
                ...
                if ( adjacentVertex ) {
                    ...
                    // TODO: have to look for an incomplete corner pixel...
        ...
        function fillArray(arr, coord, wrong) {

            if ( arr.length === edgeThreshold ) return arr;
            
            const newCoord = getAdjacentPoint(coord, wrong);
            if ( newCoord === false ) return arr;
            arr.push(newCoord);
            return fillArray(arr, JSON.stringify(newCoord), coord);
        }
        ...
        function findVertex(trailingCar, leadCar, lastIndex, isMaybeVertex) {
            ...
            // Get a new point.
            const newPt = getAdjacentPoint(JSON.stringify(leadCar[lastIndex]), {wrong: leadCar[lastIndex-1]});
            if ( newPt === false ) {
        ...
        for (const [vertexCoord, value] of this.vertices) {
            ...
            // Create the first array (trailing car) leading away from the selected vertex.
            fillArray(trailingCar, vertexCoord, null);

            if ( trailingCar.length === edgeThreshold ) {
                // Create the second array (leading car) leading away from the first array (trailing car)
                fillArray(leadCar, JSON.stringify(trailingCar[lastIndex]), JSON.stringify(trailingCar[lastIndex - 1]));
                if ( leadCar.length === edgeThreshold ) {
                    ...
                    if ( !foundVertex ) {
                        ...
                        continue;  // Step to the next known vertex
                    }
