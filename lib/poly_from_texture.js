...
export class Texture2Polygon {
    ...
    static getVertices() {
        ...
        function _getAdjacentPoint(vertex, wrong =  null) {
            ...
            for (const offsetX of pixelOffset) {
                for (const offsetY of pixelOffset) {
                    ...
                    const newCoord = [vertex[0] + offsetX, vertex[1] + offsetY];
        ...
        function fillArray(arr, options = {coord: null, wrong: null}) {
            if ( arr.length === 0 && options.coord === null) log(false, "fillArray function missing coordinates for first pass.");
            if ( arr.length === edgeThreshold ) return arr;
            
            const newCoord = arr.length === 0 ? getAdjacentPoint(options.coord) : getAdjacentPoint(arr[arr.length - 1], {wrong: arr[arr.length - 2]});
            if ( newCoord === false ) return arr;
            arr.push(newCoord);
            return fillArray(arr);
        }

        function iterateDistancesFromLine() {
        ...
        function findVertex() {
            ...
            if ( slopeL !== slopeT ) {
                ...
                const distCheck = iterateDistancesFromLine();
                if ( distCheck ) {
            ...
            // Get a new point.
            const newPt = getAdjacentPoint(leadCar[lastIndex], {wrong: leadCar[lastIndex - 1]});
            if ( newPt === false ) {
        ...
        this.vertices.forEach((value, vertexCoord) => {
            ...
            if ( trailingCar.length === edgeThreshold ) {
                ...
                fillArray(leadCar, {wrong: trailingCar[lastIndex - 1]});
                if ( leadCar.length === edgeThreshold ) {

                    // Now we start trying to find a vertex.
                    const foundVertex = findVertex();
                    if ( !foundVertex ) return  // Step to the next known vertex

                    //TODO: THIS IS A VERTEX!
                    ui.notification.error("Add code to create a VERTEX!.  See getVertices() function.");

                } else {
                    // Not enough points?  Check if there's a vertex
                    ui.notification.error("Add code to check for vertex (leadCar).  Fewer points than edgeThreshold.");
                }
            } else {
                // Not enough points?  Check if there's a vertex
                ui.notification.error("Add code to check for vertex (ptsBetweenVertices).  Fewer points than edgeThreshold.");
            }
