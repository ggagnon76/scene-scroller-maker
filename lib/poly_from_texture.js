...
export class Texture2Polygon {
     ...
     * This method gets called by a class instance to generate a drawing polygon from the
    ...
    async generateDrawingPolygon() {
        ...
        if ( !confirm ) return false;
        confirm = await #removeInteriorWallsVertices();
        if ( !confirm ) return false;
        return true;
    }
    ...
    #compressPixelData() {
        ...
        for (let x = 0; x < d.sceneWidth; x++) {
            ...
            if ( !isBlack ) {
                ...
                toUpdate[toUpdate.length-1].push(d.sceneHeight);
        ...
        for (let y = 0; y < d.sceneHeight; y++) {
            ...
            if ( !isBlack ) {
                ...
                toUpdate[toUpdate.length-1].push(d.sceneWidth);
    ...
    async #removeInteriorWallsVertices() {
        ...
        for (const vertex of this.vertexCoords) {
            if ( !this.perimeterPixels.has(vertex) ) this.vertexCoords.delete(vertex);
        }
        ...
        if ( this.#isDebugging ) {
            ...
            const confirm  = await Dialog.confirm({
                title: "Review Vertices after interior verticesremoved.",
                content: "<p>Click YES to continue if vertices are correct.</p><p>Otherwise, click NO to abort execution.</p>",
                yes: () => {return true},
                no: () => {return false}
            })
