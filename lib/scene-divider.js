...
import { _createLinks, _embedLinkFlags, deleteDivLinks } from "../dev/dev.js";
...
export class SSM_SceneDivider extends FormApplication {
    ...
    /** ********************************************************************************************************************** */
    /** Optional, call after instantiation: load saved flag data   */
    /** Optional, call after load saved flag data: divide the Scene */
    /** ********************************************************************************************************************** */
     ...
     * @param {string|boolean}  options.folderInScenes  Optional: Name of a folder (existing or not) in the Scenes tab where sub-scenes will be saved. 
     * @returns {void}
     */
    async divideScene({imgPath = false, newCompendium = false, compendiumName = false, folderInScenes = false} = {}) {

        // Define where image files will be saved
        if ( !imgPath ) {
            this.path = game.settings.get(ModuleName, "defaultImagePath");
            if ( this.path === "" ) {
                ui.notifications.error(game.i18n.localize("SSM.SceneDividerUI.NoImgPathErr"));
                this.close();
                return;
            }
        } else this.path = imgPath;

        // Sort through possible scene save locations logic
        if ( newCompendium && !compendiumName) {    // Flagged for new compendium, but no compendium name provided
            ui.notifications.error(game.i18n.localize("SSM.SceneDividerUI.NoNewCompNameErr"));
            this.close();
            return;
        }

        if ( newCompendium && getCompendiumPack(compendiumName) ) {     // Flagged for a new compendium, but compendium already exists
            ui.notifications.error(game.i18n.localize("SSM.SceneDividerUI.AlreadyCompNameErr"));
            this.close();
            return;
        }

        if ( !newCompendium && compendiumName && !getCompendiumPack(compendiumName) ) {     // No new compendium, but the provided compendium name doesn't exist.
            ui.notifications.error(game.i18n.localize("SSM.SceneDividerUI.NoExistingCompErr"));
            this.close();
            return;
        }

        if ( compendiumName && folderInScenes ) {   // Destination in folder and destination in compendium at the same time.
            ui.notifications.error(game.i18n.localize("SSM.SceneDividerUI.FolderAndCompendiumErr"));
            this.close();
            return;
        }

        let pack = undefined;
        let choice = "4";       // Choice = 4 && pack = undefined:  Sub-scenes saved in Scenes Tab

        if ( newCompendium ) {  // Choice = 3: Will create a new compendium
            choice = "3";
        }

        if ( !newCompendium && compendiumName ) pack = getCompendiumPack(compendiumName);   // No new compendium, fetch provided compendium pack name.

        let folder = false;
        if ( folderInScenes ) folder = game.folders.find(f => f.type === "Scene" && f.name === folderInScenes);
        if ( folderInScenes && !folder ) folder = await Folder.create({type: "Scene", name: folderInScenes});
        let folderId = "";
        if ( folderInScenes ) folderId = game.folders.getName(folderInScenes).id;

        // Processing choice
        switch(choice) {
            case "3":
                // Create the compendium if it doesn't already exist
                const confirm = await this.createCompendium(compendiumName);
                if ( !confirm ) {
                    ui.notifications.error(game.i18n.localize("SSM.SceneDividerUI.CompCreateFailErr"));
                    return;
                };
                pack = getCompendiumPack(compendiumName);
            case "4":
                if ( !folder ) await this._divideScenes.apply(this, [{pack: pack}]);
                else await this._divideScenes.apply(this, [{pack: pack, folderId: folderId}]);
                this.close();
                break;
        }
    }
    ...
    async _processSelections(choice, sceneSaveLoc) {
        ...
        switch(choice) {
            ...
            case "4":
                this._divideScenes.apply(this, [{pack: pack}]);
                break;
            case "5":
    ...
    async _divideScenes({pack = undefined, folderId = undefined} = {}) {
        ...
        await this.addSubScenesToLoc({pack : pack, folderId : folderId});
    ...
    restoreWalls() {
        ...
        for (const wall of seeThruWalls) {
            if ( wall.document.flags[ModuleName].hasOwnProperty("sight") ) {
                wall.document.sight = wall.document.flags[ModuleName].sight;
            } else {
                wall.document.ds = wall.document.flags[ModuleName].ds;
            };
            delete wall.document.flags[ModuleName];
        }
    }
    ...
    _divContinue() {
        ...
        // Make all see thru walls block vision
        this.convertWalls();
    ...
    _deleteDiv(drwID) {
        ...
        deleteDivLinks.apply(this, [drwID]);
     ...
     */
    async addSubScenesToLoc({pack = undefined, folderId = undefined} = {}) {
        for (const division of this.divisions) {
            // Modifiy data
            const newData = {
                folder: folderId,
            ...
            let newScene;
            if ( !pack ) newScene = await Scene.create(newData);
