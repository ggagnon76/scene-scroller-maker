...
export class SSM_SceneDivider extends FormApplication {
    ...
    createLinks(div) {
        ...
        // Now for edges that are not walls
        let polyCoords = div.polygonVertexCoordinates;
        polyCoords = polyCoords.map(c => {
            return [c[0] + div.boundingBox.sceneX, c[1] + div.boundingBox.sceneY]
        })
        for (let i=0; i < polyCoords.length - 1; i++) {
            const wallCoord = JSON.stringify(polyCoords[i]) + "," + JSON.stringify(polyCoords[i+1]);
            const backwardWallCoord = JSON.stringify(polyCoords[i+1]) + "," + JSON.stringify(polyCoords[i]);

            let noWall = true;
            for (const [id, wall] of div.instanceWalls) {
                if ( !noWall ) continue;
                if ( div.omitWalls.has(wall) ) continue;
                const wallCoords = wall.coords;
                const wc = JSON.stringify([wallCoords[0], wallCoords[1]]) + "," + JSON.stringify([wallCoords[2], wallCoords[3]]);
                if ( wc === wallCoord || wc === backwardWallCoord ) noWall = false;
            }

            if ( noWall ) {
                // Start looking at other divs to see if they also have these coordinates
                for (const division of this.divisions) {
                    if ( div === division ) continue;
                    let division_polyCoords = division.polygonVertexCoordinates;
                    division_polyCoords = division_polyCoords.map(c => {
                        return [c[0] + division.boundingBox.sceneX, c[1] + division.boundingBox.sceneY]
                    })
                    division_polyCoords = JSON.stringify(division_polyCoords);
                    if (
                        division_polyCoords.includes(wallCoord) ||
                        division_polyCoords.includes(backwardWallCoord)
                    ) {
                        // This division has a common edge that is not defined by a wall.
                        const linkID = div.drawingID.slice(8) + division.drawingID.slice(-8)
                        if ( this.links.has(linkID) ) continue;
                        const d = canvas.dimensions;
                        const wallCoords = [polyCoords[i][0] + d.paddingX, polyCoords[i][1] + d.paddingY, polyCoords[i+1][0] + d.paddingX, polyCoords[i+1][1] + d.paddingY];
                        const midpoint = {
