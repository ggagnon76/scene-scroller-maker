...
import { convertToBlob,  getCompendiumPack} from "./functions.js";
...
export class SSM_SceneDivider extends FormApplication {
    ...
    async getData() {
        ...
        const returnObj = {
            ...
            hasDefaultImgPath: await this.hasImgPath(),
    ...
    async _processSelections(choice, compendiumName) {
        // pre-processing
        if ( choice === "3" ) {
    ...
    async isDefaultAndNotSet() {
        if (  await this.hasImgPath() && !this.pathIsSet ) return true;
        return false;
    }
    ...
    /**
     * Logic to create a sub-scene in the selected location.
     */
