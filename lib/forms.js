    ...
    async embedLinkFlags(pack) {
        for (const [id, link] of this.links) {
            ...
            for (const div of link.divs) {
                ...
                const subSceneFlagData = subScene.getFlag(ModuleName, "LinkedTiles") || [];
                const vector = {
                    x: div.boundingBox.sceneX - otherDiv.boundingBox.sceneX,
                    y: div.boundingBox.sceneY - otherDiv.boundingBox.sceneY
                }
                const otherScnID = pack.index.getName(otherDiv.subSceneName)._id;
                const otherUUID = `Compendium.${clctn}.${otherScnID}`;
                subSceneFlagData.push({SceneUUID: otherUUID, Vector: vector});
                await subScene.setFlag(ModuleName, "LinkedTiles", subSceneFlagData);
            }
        }
    }
}
