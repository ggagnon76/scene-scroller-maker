...
export class SSM_SceneDivider extends FormApplication {
    constructor() {
        ...
        this.vertices = new Set();          // Set of coordinates extracted from clockwiseSweepPolygon (see refreshVision())
    ...
    refreshVision() {
        ...
        // Save the vertices, which share the coordinates of their respective walls.
        for (let i=0; i < this.vision.los.points.length - 1; i += 2) {
            this.vertices.add(JSON.stringify({x: this.vision.los.points[i], y: this.vision.los.points[i+1]}));
        }
    }
        ...
        for (const scene of pack.contents) {
            ...
            for (const [id, wall] of division.instanceWalls) {
                const d = new WallDocument({
                    c: [    wall.document.c[0] - division.boundingBox.x,
                            wall.document.c[1] - division.boundingBox.y,
                            wall.document.c[2] - division.boundingBox.x,
                            wall.document.c[3] - division.boundingBox.y
                    ],
                    dir: wall.document.dir,
                    door: wall.document.door,
                    ds: wall.document.ds,
                    light: wall.document.light,
                    move: wall.document.move,
                    sight: wall.document.sight,
                    sound: wall.document.sound,
                    _id: wall.id
                }, {parent: scene});
                wallArray.push(d);
            }
