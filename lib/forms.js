...
export class SSM_SceneDivider extends FormApplication {
    ...
    /**
     * Override the close method to add more logic.
    ...
    async saveSubSceneTextureToFile(division) { 
        ...
        mask.drawPolygon(division.pixiPolygon);
        mask.endFill();
    ...
    async cullPlaceablesInPack(pack) {
        for (const scene of pack.contents) {
            ...
            const cullPlaceables = (placeables) => {
                ...
                if ( placeables === "walls" ) {
                    for (const wall of placeablesCopy) {
                        const pt1 = new ClipperLib.IntPoint(wall.c[0], wall.c[1]);
                        const pt2 = new ClipperLib.IntPoint(wall.c[2], wall.c[3]);

                        if (    ClipperLib.Clipper.PointInPolygon(pt1, division.clipperPathFinal[0]) === 0 ||
                                ClipperLib.Clipper.PointInPolygon(pt2, division.clipperPathFinal[0]) === 0 ) continue;

                        wall.c[0] -= division.boundingBox.x;
                        wall.c[1] -= division.boundingBox.y;
                        wall.c[2] -= division.boundingBox.x;
                        wall.c[3] -= division.boundingBox.y;

                        updateArray.push(wall);
                    }
                } else {
            ...
            const walls = cullPlaceables("walls");
...
export class SSM_CropImage extends FormApplication {
    ...
    validateWalls() {
        ...
        for (const wall of walls) {
            wallCoords.push(JSON.stringify([wall.document.c[0], wall.document.c[1]]));
            wallCoords.push(JSON.stringify([wall.document.c[2], wall.document.c[3]]));
        }
