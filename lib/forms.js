...
export class SSM_SceneDivider extends FormApplication {
    constructor() {
        ...
        this.polygonOffsetByPixels = 2;     // Grow the polygon to make sure there are no gaps when the pieces are reassembled.
    ...
    /**
     * Override the close method to clean up all the implemented changes.
    ...
    _divContinue() {
        ...
        // Replace the click left event handler to create a VisionSource and update perception.
        ...
        // Replace the drag left move handler to do 2 things:
        // 1) prevent this handler from trying to update the perception too often, ie: throttle
        // 2) update the perception to 'paint' the area visible from the moving mouse cursor.
    ...
    async saveSubSceneTextureToFile(division) {
        ...
        co.Execute(solution, this.polygonOffsetByPixels * scale);
        ...
        let tex, sprite;
        if ( canvas.scene.background.src !== null ) {
            tex = await loadTexture(canvas.scene.background.src);
        ...
        const texture = PIXI.RenderTexture.create({
            width: division.boundingBox.width + division.boundingBox.offsetX + this.polygonOffsetByPixels,
            height: division.boundingBox.height + division.boundingBox.offsetY + this.polygonOffsetByPixels,
    ...
    async cullPlaceablesInPack(pack) {
        for (const scene of pack.contents) {
            ...
            const cullPlaceables = (placeables) => {
                const updateArray = []
                if ( placeables === "walls" ) {
                    const placeablesCopy = division.wallsInDiv;
                    for (const wall of placeablesCopy) {
                    ...
                    for (const placeable of placeablesCopy) {    // is a Map
                        ...
                        if ( ClipperLib.Clipper.PointInPolygon(pt1, division.clipperPathFinal[0]) === 0 ) continue;
    ...
    createLinks(div) {
        ...
        for (const division of this.divisions) {
            ...
            cpr.AddPath(div.clipperPathFinal[0], ClipperLib.PolyType.ptSubject, false); // false here means this is a line, not a polygon
            cpr.AddPath(division.clipperPathFinal[0], ClipperLib.PolyType.ptClip, true); // Clip has to be true (polygon)

            const clipType = ClipperLib.ClipType.ctIntersection;

            const result = new ClipperLib.PolyTree();
            cpr.Execute(clipType, result)

            const edges = ClipperLib.Clipper.OpenPathsFromPolyTree(result)[0];
            if ( edges === undefined ) continue;

            // Need to check if there are walls on this edge.  If there is, continue.

            const wallCoords = []
            for (const edge of edges) {
                wallCoords.push(edge.X, edge.Y);
            }
            const midpoint = {
...
export class SSM_ConvertImage extends FormApplication {
    constructor(resolve, batch = false, compression = true, submitOnChange = false) {
        super();
        this.result = resolve;
        this.batch = batch;
...
export class SSM_ResizeImage extends FormApplication {
    ...
    getData() {
        return {
            tileWidth : Math.round(this.tile.width),
            tileHeight : Math.round(this.tile.height)
        }
    }
