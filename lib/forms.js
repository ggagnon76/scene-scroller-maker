...
export class SSM_SceneDivider extends FormApplication {
    ...
    async embedLinkFlags(pack) {
        ...
        for (const div of this.divisions) {
            minX = div.boundingBox.sceneX;
            minY = div.boundingBox.sceneY;
            maxX = div.boundingBox.sceneX + div.boundingBox.width;
            maxY = div.boundingBox.sceneY + div.boundingBox.height;
            const scn_id = pack.index.getName(div.subSceneName)._id;
            const uuid = `Compendium.${clctn}.${scn_id}`;
            const subScene = await fromUuid(uuid);
        ...
        /* Step 3: Iterate every sub-scene to calculate sub-scene coordinates relative to minX & minY and save them in flags */
        for (const div of this.divisions) {
            const scn_id = pack.index.getName(div.subSceneName)._id;
            const uuid = `Compendium.${clctn}.${scn_id}`;
            const subScene = await fromUuid(uuid);
            ...
            for (const data of subSceneFlagData) {
                for (const division of this.divisions) {
                    ...
                    data[subSceneChildrenKeys[1]] = {
                        x: division.boundingBox.sceneX - minX,
                        y: division.boundingBox.sceneY - minY
                    }
                    break;
                }
            }
