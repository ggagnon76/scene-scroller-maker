...
export class SSM_SceneDivider extends FormApplication {
    ...
    async _updateObject(event, formData) {
        ...
        const data = canvas.scene.toCompendium(pack, {})
        await this.addSubScenesToCompendium(data, pack);
     ...
     */
    async addSubScenesToCompendium(sceneData, pack) {
        for (const division of this.divisions) {
            // Modifiy data
            const newData = {
            ...
            foundry.utils.mergeObject(newData, sceneData, {overwrite: false});
    ...
    /**
     * Logic to remove placeables that are outside the image of the sub-scene,
     * and normalizes the remaining placeables coordinates relative to the new sub-scene origin.
     */
    async cullPlaceablesInPack(pack) {
        // Remove placeables outside of the bounds of each sub-scene.
        for (const scene of pack.contents) {
            // Find the division for this scene...
            const div = this.divisions.filter(d => d.subSceneName === scene.name)[0];
            // Create a PIXI polygon for this div
            // Use to determine if a placeables coordinate is contained within it.
            const poly = generatePixiPolygon(div.polygonVertexCoordinates);

            // Start with walls, since we already have that info.
            // But first have to ajust the coordinates to reflect the coordinates of the sub-scene
            const wallArray = [];
            for (const wall of scene.data.walls) {          // scene.data.walls is a Map
                if ( div.instanceWalls.has(wall.id) ) {    // div.instanceWalls is a Map
                    const d = new WallDocument({
                        c: [    wall.data.c[0] -= div.boundingBox.x,
                                wall.data.c[1] -= div.boundingBox.y,
                                wall.data.c[2] -= div.boundingBox.x,
                                wall.data.c[3] -= div.boundingBox.y
                        ],
                        dir: wall.data.dir,
                        door: wall.data.door,
                        ds: wall.data.ds,
            ...
            const cullPlaceables = (placeables) => {
                const updateArray = []
                for (const placeable of scene.data[placeables]) {    // is a Map
                    const x = placeable.data.x - (div.boundingBox.x - div.boundingBox.offsetX);
                    const y = placeable.data.y - (div.boundingBox.y - div.boundingBox.offsetY);
                    if ( poly.contains(x, y) ) {
                        const updateObj = {
                            _id: placeable.id,
                            x: placeable.data.x - div.boundingBox.x,
                            y: placeable.data.y - div.boundingBox.y
                        }
            ...
            // Deleted the polydrawings, which removes them from the database.  Because of the way
            // the update was done, they were not removed from memory...
            for (const division of this.divisions) {
                scene.data.drawings.delete(division.drawingID);
            }
            const drawingArray = cullPlaceables("drawings");
            const lightsArray = cullPlaceables("lights");
            const notesArray = cullPlaceables("notes");
            const soundsArray = cullPlaceables("sounds");
            const templatesArray = cullPlaceables("templates");
            const tilesArray = cullPlaceables("tiles");
            const tokensArray = cullPlaceables("tokens");

            await scene.update({
                walls: wallArray,
                drawings: drawingArray,
                lights: lightsArray,
                notes: notesArray,
                sounds: soundsArray,
