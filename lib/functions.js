    ...
    if ( result ) {
        ...
        for (const drawing of canvas.drawings.placeables) {
            ...
            if ( drawing.document.shape.type === "r" ) {        // rectangle
                drawingPath = [
                    drawing.document.x, drawing.document.y,                 // Top Left Corner
                    drawing.document.x + drawing.document.shape.width,
                    drawing.document.y,                                     // Top Right Corner
                    drawing.document.x + drawing.document.shape.width,
                    drawing.document.y + drawing.document.shape.height,     // Bottom Right Corner
                    drawing.document.x,
                    drawing.document.y + drawing.document.shape.height      // Bottom Left Corner
                ]
            }
                ...
                if ( game.modules.get('_dev-mode')?.api?.getPackageDebugValue(ModuleName) ) {
                    await displayPoints(drawingPath);
                }

            }
            else if ( drawing.document.shape.type === "p" ) {   // polygon
                let evenOdd = true;
                for (const p of drawing.document.shape.points) {
                    if ( evenOdd ) drawingPath.push(p + drawing.document.x)
                    else drawingPath.push(p + drawing.document.y)
                    evenOdd = !evenOdd
                }
            }
            else {
        ...
        canvas.app.renderer.render(tempContainer, texture, undefined);
...
function ellipseToPolygon(firstAxis, secondAxis, pointQty, sceneX, sceneY, rotation) {
    ...
    if ( angleDeg !== 0 ) {
        ...
        for (let i = 0; i < pointsArray.length - 2; i+= 2) {
            const tempPoint = Math.round((pointsArray[i] * Math.cos(angleRad) - pointsArray[i+1] * Math.sin(angleRad)) * 1000) / 1000;
            pointsArray[i+1] = Math.round((pointsArray[i+1] * Math.cos(angleRad) + pointsArray[i] * Math.sin(angleRad)) * 1000) / 1000;
    ...
    const halfWidth = firstAxis > secondAxis ? firstAxis / 2 : secondAxis / 2;
    const halfHeight = firstAxis > secondAxis ? secondAxis / 2 : firstAxis / 2;
    for (let i = 0; i < pointsArray.length - 2; i+= 2) {
...
async function displayPoints(points) {
    ...
    for ( let i = 0; i < points.length - 2; i+=2) {
        ...
        dot.drawCircle(points[i], points[i+1], 5);
        dot.endFill();
