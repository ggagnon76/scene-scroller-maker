...
import { SSM_SceneDivider, SSM_ConvertImage, SSM_ResizeImage, SSM_CropImage, PickScene, polygonOffsetByPixels } from "./forms.js";
...
async function saveImgAsWebP(img, imgPath, compression) {
    ...
    // Save to disk
    const path = decodeURIComponent(imgPath.replace(fileNameExtract, ""));
...
async function convertImgFile(path = null) {
    ...
    const imgData = await new Promise((resolve) => {
        return new SSM_ConvertImage(resolve, {path: decodeURIComponent(path)}).render(true);
    })
