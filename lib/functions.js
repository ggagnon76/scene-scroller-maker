...
async function convertBackground() {
    ...
    const foreground = canvas.scene.foreground?.src;
    let newBackground = undefined;
    let newForeground = foundry.utils.duplicate(canvas.scene.foreground);

    if ( background === undefined) {
        ui.notifications.warn(game.i18n.localize("SSM.ConvertBackground.noBackground"));
        return;
    } else {
        newBackground = await convertImgFile(background);
    }

    if ( foreground !==  undefined) {
        newForeground.src = await convertImgFile(foreground);
    } else newForeground = null;
...
async function saveImgAsWebP(img, imgPath, compression) {
    ...
    const check = await FilePicker.upload("data", path, file);
