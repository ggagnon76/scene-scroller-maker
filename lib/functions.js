...
import { SSM_SceneDivider, SSM_ConvertImage } from "./forms.js";
...
async function convertImgFile() {
    ui.notifications.info("Clicked convert image file button");
    ...
    if ( fileExtension === "webp" ) {
        ui.notifications.info("File is already in .webp format.")
        return;
    }

    const tex = await loadTexture(imgData.imagePath);
    const image = canvas.app.renderer.extract.base64(new PIXI.Sprite(tex), "image/webp", imgData.compression);
    const blob = convertToBlob(image);

    // Save to disk
    const fileName = fileNameExtract.replace("." + fileExtension, ".webp");
    const path = imgData.imagePath.replace(fileNameExtract, "");
    const file = new File([blob], fileName, {type: 'image/webp'});
    await FilePicker.upload("data", path, file);
}

function convertImgsInFolder() {
    ui.notifications.info("Clicked convert images in folder button");
}

function resizeImage() {
    ui.notifications.info("Clicked resize image button");
}
