...
export async function processFlaggedDivisionData({imgPath = false, newCompendium = false, compendiumName= false, folderInScenes = false} = {}) {
    ...
    await ssmSD.loadFlagData();
    await ssmSD.divideScene({imgPath: imgPath, newCompendium : newCompendium, compendiumName : compendiumName, folderInScenes: folderInScenes});
}

function isActiveScene() {
    if ( !canvas.scene ) {
        ui.notifications.warn(game.i18n.localize("SSM.hasNoCanvasScene"));
        return false
    }
    return true
}
