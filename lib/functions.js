...
export async function saveImgAsWebP(fp, compression, fn = false, texture = false, force = false) {
    ...
    if ( fp.activeSource === "s3" ) {
        const discard = fp.request.split("/").pop();
        const s3path = fp.request.replace(discard, "");
    ...
    if ( result ) {
        ...
        for (const drawing of canvas.drawings.placeables) {
            ...
            if ( drawing.document.shape.type === "r" ) {        // rectangle
                ...
                let path = PolygonMesher.getClipperPathFromPoints(drawingPath);
                if ( !ClipperLib.Clipper.Orientation(path) ) {
                    drawingPath = flattenClipperPoints(path.reverse());
                }
                if ( game.modules.get('_dev-mode')?.api?.getPackageDebugValue(ModuleName) ) {
                    await displayPoints(drawingPath, 10);
                }
        ...
        const unionSolution = clipperIntersection(getSceneBoundsAsClipper(), unionPaths);
...
export function clipperUnion(subject, clips) {
    if ( !Array.isArray(clips[0]) ) clips = [clips];
    const union = new ClipperLib.Clipper();
    const solution = new ClipperLib.Paths();
    union.AddPath(subject, ClipperLib.PolyType.ptSubject, true);
    union.AddPaths(clips, ClipperLib.PolyType.ptClip, true);

    const clipType = ClipperLib.ClipType.ctUnion;
    const fillType = ClipperLib.PolyFillType.pftPositive;
    union.Execute(clipType, solution, fillType, fillType);

    if ( solution.length > 1 ) {
        ui.notifications.i18n(game.i18n.localize("SSM.T2P_Errors.ClipperIntersectErr"));
    }

    return solution[0];
}

export function clipperIntersection(subject, clips) {
    if ( !Array.isArray(clips[0]) ) clips = [clips];
    const intersect = new ClipperLib.Clipper();
    const solution = new ClipperLib.Paths();
    intersect.AddPath(subject, ClipperLib.PolyType.ptSubject, true);
    intersect.AddPaths(clips, ClipperLib.PolyType.ptClip, true);

    const clipType = ClipperLib.ClipType.ctIntersection;
    const fillType = ClipperLib.PolyFillType.pftPositive;

    intersect.Execute(clipType, solution, fillType, fillType);

    if ( solution.length > 1 ) {
        ui.notifications.warn(game.i18n.localize("SSM.T2P_Errors.ClipperIntersectErr"));
    }

    return solution[0];
}

export function clipperDifference(subject, clips) {
    if ( !Array.isArray(clips[0]) ) clips = [clips];
    const difference = new ClipperLib.Clipper();
    const solution = new ClipperLib.Paths();
    difference.AddPath(subject, ClipperLib.PolyType.ptSubject, true);
    difference.AddPaths(clips, ClipperLib.PolyType.ptClip, true);

    const clipType = ClipperLib.ClipType.ctDifference;
    const fillType = ClipperLib.PolyFillType.pftPositive;

    difference.Execute(clipType, solution, fillType, fillType);

    if ( solution.length > 1 ) {
        ui.notifications.warn(game.i18n.localize("SSM.T2P_Errors.ClipperDiffErr"))
    }
