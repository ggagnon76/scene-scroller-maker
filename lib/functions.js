...
async function cropImage() {
    ...
    //Use walls to enclose area to crop.  Can be polygon
    await new Promise((resolve) => {
        return new SSM_CropImage(resolve).render(true);
    })

    const walls = canvas.walls.placeables;
    const coordsInOrder = [JSON.stringify([walls[0].data.c[0], walls[0].data.c[1]])];
    do {
        const nextWall = walls.filter(w => JSON.stringify([w.data.c[2], w.data.c[3]]) === coordsInOrder[coordsInOrder.length - 1])[0];
        coordsInOrder.push(JSON.stringify([nextWall.data.c[0], nextWall.data.c[1]]));
    } while ( coordsInOrder[0] !== coordsInOrder[coordsInOrder.length - 1] )
    
    const smallestX = coordsInOrder.reduce((pv, cv) => Math.min(pv, JSON.parse(cv)[0]), canvas.dimensions.width);
    const smallestY = coordsInOrder.reduce((pv, cv) => Math.min(pv, JSON.parse(cv)[1]), canvas.dimensions.width);
    const largestX = coordsInOrder.reduce((pv, cv) => Math.max(pv, JSON.parse(cv)[0]), 0);
    const largestY = coordsInOrder.reduce((pv, cv) => Math.max(pv, JSON.parse(cv)[1]), 0);
    const bounds = {
        x: smallestX,
        y: smallestY,
    ...
    const normalizedCoords = coordsInOrder.map(coord => {
        return [JSON.parse(coord)[0] - bounds.x, JSON.parse(coord)[1] - bounds.y]
    })
    ...
    const fileNameExtract = imgData.imagePath.split("/").pop();
    const fileExtension = fileNameExtract.split(".").pop();
    const fileName = imgData.imagePath.replace("." + fileExtension, "_cropped.webp")
    saveImgAsWebP(texture, fileName, 1)
    if ( oldScene !== null ) oldScene.update({active: true});
    scene.delete();
}
