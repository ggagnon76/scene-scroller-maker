...
import { SSM_SceneDivider, SSM_ConvertImage, SSM_ResizeImage, SSM_CropImage, PickScene } from "./forms.js";
...
async function convertBackground() {
    ...
    let newForeground = canvas.scene.foreground;

    if ( background === undefined) {
        ui.notifications.warn(game.i18n.localize("SSM.ConvertBackground.noBackground"));
        return;
    } else {
        if ( !background.includes("webp") ) newBackground = await convertImgFile(background);
    }

    if ( foreground !==  null && !foreground.includes("webp") ) newForeground = await convertImgFile(foreground);

    if ( !background.includes("webp") || (foreground !== null && !foreground.includes("webp")) )
        canvas.scene.update({background: {src: newBackground}, foreground: newForeground})
}
...
async function saveImgAsWebP(img, imgPath, compression) {
    ...
    if ( fileExtension === "webp") return undefined;
...
async function convertImgFile(path = null) {
    // Spawn application with quality slider and filepicker button
    const imgData = await new Promise((resolve) => {
        return new SSM_ConvertImage(resolve, {path: path}).render(true);
    })

    const tex = await loadTexture(imgData.imagePath);
    return await saveImgAsWebP(tex, imgData.imagePath, imgData.compression); 
}
...
async function resizeImage(path = null) {
    ...
    if ( imgData.imagePath === null ) {
        imgData = await new Promise((resolve) => {
            return new SSM_ConvertImage(resolve, {compression: false, submitOnChange: true}).render(true);
        })
    }
    ...
    if ( result ) {
        ...
        fileName = imgData.imagePath.replace("." + fileExtension, "_RESIZED_" + widthSquares + "x" + heightSquares + "@" + d.size + "ppi." + fileExtension);
        fileName = await saveImgAsWebP(texture, fileName, result.compression)
    }
...
async function cropImage() {
    ...
    if ( result ) {
        ...
        if ( unionSolution === false ) {
            ...
            const fileName = imgData.imagePath.replace("." + fileExtension, "_CROPPED_" + widthSquares + "x" + heightSquares + "@" + d.size + "ppi." + fileExtension);
            saveImgAsWebP(texture, fileName, result.compression)
        }
    }
...
async function linkScenes() {
    ...
    // The background images are local memory only tiles wihtout event handlers (can't be selected).
    ...
    // Then spawn a confirmation dialog.
    debugger;
}
...
export function offsetPolygon(polygon, offsetBy, scale) {
    const dupe = JSON.parse(JSON.stringify(polygon));
    const poly = Array.isArray(dupe[0]) ? dupe[0] : dupe;
    ...
    co.Execute(solution, offsetBy * scale);
    let final = Array.isArray(solution[0]) ? solution[0] : solution;
    ClipperLib.JS.ScaleDownPath(final, scale);
    return final;
}
