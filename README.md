...
The V2.0 release enhances existing functionality introduced with V1.0 by:
 ...
 - adding module settings to define a default folder to save images generated by Scene Scroller Maker 
 - to provide the option to create scenes in the core Scene Folder instead of a compendium

 This initial V2.0 release is being pushed out early to fix a bad .CSS definition that is clobbering other module's forms. (Sorry...)  Follow up updates will implement further refinements:
 - the default image folder setting currently is only being invoked by the 'divide scene' tool.  The other image tools will inherit this option as well.
 - adding module settings to hide button bar added by Scene Scroller Maker with a right-click menu button to toggle the setting in the navigation bar for the active map.
 - add option to save the boundary polygons defining sub-scenes to scene flags, to be able to quickly generate sub-scenes at a later time

NOTE:  The sequence of operations for the 'divide scene' tool has changed from V1.0:

#### How to use 'divide scene' tool in V2.0?
The form that is presented will provide some basic instruction.  The basic sequence is:
1) Create one or many new division(s) (these become sub-scenes) while providing each with a name,
2) Explore the area for each division to encompass by left-clicking and dragging the mouse over the map,
3) Click NEXT to select a folder where the tool will save all the image pieces for the new sub-scenes,
4) Click NEXT to select a location where the sub-scenes will be created,
5) Click PREVIOUS or NEXT to revise any of the above steps before submitting,
6) Submit the form to create the sub-scenes at the chosen location.

NOTE:  While bug testing this release, it was discovered the crop tool has a bug in it that is not resolved yet.  In order to avoid this bug for the time being, you must draw your overlapping shapes from the top down, or from right to left.  Doing otherwise will cause the image to have holes in them, or the crop will fail with an error message stating the shapes were not overlapping.
