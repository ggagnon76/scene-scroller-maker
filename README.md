...
The V2.0 release enhances existing functionality introduced with V1.0 by:
 ...
 - adding module settings to define a default folder to save images generated by Scene Scroller Maker (all image generating tools)
 - adding the option to create scenes in the core Scene Folder instead of a compendium (divide scene tool)
 - adding the option to save the boundary polygons defining sub-scenes to scene flags, to be able to quickly generate sub-scenes at a later time

Follow up updates will implement further refinements:
...
1) Create a new division and provide it with a name (these become sub-scenes),
2) Left-click and drag to explore the area to be encompassed by the division,
3) Repeat steps 1 & 2 as required,
4) Click NEXT to select a folder where the tool will save all the cropped images for the new sub-scenes,
5) Click NEXT to select a location where the sub-scenes will be created,
6) Click PREVIOUS or NEXT to revise any of the above steps before submitting,
7) Submit the form to create the sub-scenes at the chosen location.
